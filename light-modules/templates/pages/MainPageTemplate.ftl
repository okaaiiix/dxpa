...
<html xml:lang="${cmsfn.language()}" lang="${cmsfn.language()}">
  <head>
    ...
    [#-- To load resources you can link them manually (e.g. line below) --]
      [#-- <link rel="stylesheet" type="text/css" href="${ctx.contextPath}/.resources/light-modules/webresources/css/bootstrap.css" media="all" /> --]
      [#-- <script src="${ctx.contextPath}/.resources/light-modules/webresources/js/jquery.js"></script> --]
    [#-- or via theme --]
    ...
    [#-- uncomment next line to use resfn templating functions to load all css which matches a globbing pattern --]
      [#-- ${resfn.css(["/light-modules/.*css"])!}--]
  </head>
  <body class="MainPageTemplate ${cmsfn.language()}">

    <div class="container">
      <h1>MainPageTemplate works!</h1>
    </div>

    [#-- use resfn to load all js which matches the globbing pattern or link resources manually or via theme --]
    [#-- ${resfn.js(["/light-modules/.*js"])!} --]
  </body>
</html>
